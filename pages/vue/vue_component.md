...
`component`란 조합하여 화면을 구성할 수 있는 화면의 특정 영역을 가리키는 블록을 의미합니다. 
<br><br>
...
전역 컴포넌트는 Vue 라이브러리를 로딩하고 나면 접근 가능한 생성자 함수 `Vue`를 이용하여 등록합니다. 
<br><br>
...
그리고 컴포넌트 태그가 실제 화면의 HTML 요소로 변환될 때 표시될 속성들을 컴포넌트 내용에 작성합니다. 여기에는
...
위의 예제를 보면 명확히 이해할 수 있습니다. 
<br><br>

전역 컴포넌트는 인스턴스를 생성할 때마다 인스턴스에 `components` 속성으로 등록할 필요가 없이 한번 등록하면 
어느 인스턴스에서든 사용할 수 있습니다. 반대로 지역 컴포넌트는 새 인스턴스를 생성할 때마다 등록해 줘야 합니다. 
당연하죠!!
